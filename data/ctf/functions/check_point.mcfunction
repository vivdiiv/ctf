...
execute if block ~1 ~1 ~ minecraft:red_banner{CustomName: "\"§r§4Red Flag§r\""} as @a[team=ctf_blue,sort=nearest,limit=1] as @s run tellraw @a [{"selector":"@s","color":"blue"},{"text":" has scored a point! ","color":"yellow"}]
# Increment score
...
# Send a message to the server that a point has been scored
execute if block ~1 ~1 ~ minecraft:blue_banner{CustomName: "\"§r§1Blue Flag§r\""} as @a[team=ctf_red,sort=nearest,limit=1] as @s run tellraw @a [{"selector":"@s","color":"dark_red"},{"text":" has scored a point! ","color":"yellow"}]
# Increment score
